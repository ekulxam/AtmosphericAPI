...
public class LivingEntityRendererMixin {
    ...
    *///?} elif >=26 {
    ...
    private <T extends LivingEntityRenderState> void renderEmpty(SubmitNodeCollector instance, Model model, Object state, PoseStack matrixStack, net.minecraft.client.renderer.rendertype.RenderType renderType, int light, int overlay, int color, TextureAtlasSprite textureAtlasSprite, int outlineColor, ModelFeatureRenderer.CrumblingOverlay crumblingOverlay, Operation<Void> original) {
        if ((LivingEntityRenderer) (Object) this instanceof EmptyModelRenderer emptyModelRenderer) {
            emptyModelRenderer.renderWithEntityData((T) state, matrixStack, instance, light, overlay, color, outlineColor);
        } else {
            original.call(instance, model, state, matrixStack, renderType, light, overlay, color, textureAtlasSprite, outlineColor, crumblingOverlay);
        }
    }
    //?} elif >=1.21.9 {
