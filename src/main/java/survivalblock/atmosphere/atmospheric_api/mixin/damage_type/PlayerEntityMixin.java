    ...
    private boolean bypassesCreativeMode(boolean original, DamageSource source, float amount) {
        return original ||
                (source.isIn(AtmosphericDamageTypeTags.BYPASSES_CREATIVE) &&
                        this.isCreative() &&
                        this.getWorld().getGameRules().getBoolean(AtmosphericDamageTypeGamerules.ALLOW_BYPASSING_CREATIVE)) ||
                (source.isIn(AtmosphericDamageTypeTags.BYPASSES_SPECTATOR) &&
                        this.isSpectator() &&
                        this.getWorld().getGameRules().getBoolean(AtmosphericDamageTypeGamerules.ALLOW_BYPASSING_SPECTATOR));
    }
}
